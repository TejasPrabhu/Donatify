...
def get_items(page, user_id):
	...
	try:
		...
		data2 = data2.replace("[","(")
		data2 = data2.replace("]",")")
		sql_select_query = """select * from items where quantity > 0 AND donor_id != {} AND category in {} order by item_id  limit 10 offset {} """.format(int(user_id), data2, int(page)*10-10)
